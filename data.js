...
//  );
//
//  -- If table already exists, add the phone column:
...
// ── Auth / Players ────────────────────────────────────────────
async function authPlayer(name, phoneNumber, billNumber) {
  const bill  = billNumber.trim();
  const phone = phoneNumber.trim();
  const nm    = name.trim();

  // Look up by bill_number
  ...
  if (existing) {
    ...
    if (existing.name.trim().toLowerCase() !== nm.toLowerCase()) {
      throw new Error('Name does not match our records for this Bill Number.');
    }
    return existing;
  }

  // New player → register with all 3 fields
  const { data: created, error: insErr } = await db
    .from('players')
    .insert({ name: nm, phone_number: phone, bill_number: bill, total_points: 0 })
    .select()
    .single();
