...
function calculatePoints(predicted, actual) {
  ...
  if (getResult(pA, pB) === getResult(aA, aB))  { points += 3; breakdown.push({ label: 'Correct result',      pts: 3 }); }
  if (pA === aA && pB === aB)                    { points += 5; breakdown.push({ label: 'Exact score',         pts: 5 }); }
  if ((pA - pB) === (aA - aB))                   { points += 2; breakdown.push({ label: 'Correct goal diff',   pts: 2 }); }
  if ((pA + pB) === (aA + aB))                   { points += 2; breakdown.push({ label: 'Correct total goals', pts: 2 }); }
  if (aB > aA && pB > pA)                        { points += 2; breakdown.push({ label: 'Underdog win bonus',  pts: 2 }); }

  return { points, breakdown };
}

// ── Auth / Players ────────────────────────────────────────────
async function authPlayer(name, billNumber) {
  const bill = billNumber.trim();
  // Look up by bill_number first
  ...
  const { data: created, error: insErr } = await db
    .from('players')
    .insert({ name: name.trim(), bill_number: bill, total_points: 0 })
    .select()
    .single();
