...
function buildMatchCard(match, result, pred, session) {
  ...
  if (match.tbd && !finalized) {
    ...
    // Open for prediction
    const savedPenalty = pred && pred.penalty_pick ? pred.penalty_pick : '';
    const penHtml = `
      <div class="penalty-pick" id="pen-${match.matchId}">
        <span class="pen-label">🥅 Who wins on penalties? <em>(optional, +2 pts)</em></span>
        <div class="pen-options">
          <label class="pen-opt ${savedPenalty === match.teamA ? 'pen-selected' : ''}">
            <input type="radio" name="pen-${match.matchId}" value="${esc(match.teamA)}"
              ${savedPenalty === match.teamA ? 'checked' : ''}> ${FLAG[match.teamA] || '🏳'} ${esc(match.teamA)}
          </label>
          <label class="pen-opt pen-opt--draw">
            <input type="radio" name="pen-${match.matchId}" value="none"
              ${savedPenalty === 'none' ? 'checked' : ''}> No Penalties
          </label>
          <label class="pen-opt ${savedPenalty === match.teamB ? 'pen-selected' : ''}">
            <input type="radio" name="pen-${match.matchId}" value="${esc(match.teamB)}"
              ${savedPenalty === match.teamB ? 'checked' : ''}> ${FLAG[match.teamB] || '🏳'} ${esc(match.teamB)}
          </label>
        </div>
      </div>`;
    bodyHtml = `
      <div class="predict-form" id="form-${match.matchId}">
        ...
        ${penHtml}
        ${pred ? `<div class="saved-pill">✓ Saved: ${pred.score_a}–${pred.score_b}${savedPenalty && savedPenalty !== 'none' ? ` · Pen: ${savedPenalty}` : savedPenalty === 'none' ? ' · No Penalties' : ''}</div>` : ''}
...
async function handleSavePred(matchId, btn) {
  ...
  try {
    const penRadio = document.querySelector(`input[name="pen-${matchId}"]:checked`);
    const penPick = penRadio ? penRadio.value : null;
    await upsertPrediction(session.id, matchId, +a.value, +b.value, penPick);
      ...
      roundMatches.forEach(match => {
        ...
        card.innerHTML = `
          <div class="adm-card-top">
            ...
            ${done ? `<span class="done-badge">✓ Final: ${result.score_a}–${result.score_b}${result.penalty_winner && result.penalty_winner !== 'none' ? ` · Pen: ${result.penalty_winner}` : ''}</span>` : ''}
          </div>
          ${done ? `<div class="adm-form adm-recalc-form">
            <span class="adm-recalc-label">⚠️ Recalculate with corrected result:</span>
            <input type="number" min="0" max="20" id="aa-${match.matchId}" class="adm-in" placeholder="${esc(teamADisplay)} goals" value="${result.score_a}">
            <span class="adm-dash">–</span>
            <input type="number" min="0" max="20" id="ab-${match.matchId}" class="adm-in" placeholder="${esc(teamBDisplay)} goals" value="${result.score_b}">
            <div class="adm-pen-row">
              <span class="adm-pen-label">Penalty winner:</span>
              <label class="adm-pen-opt"><input type="radio" name="adm-pen-${match.matchId}" value="${esc(match.tbd ? teamADisplay : match.teamA)}" ${result.penalty_winner === (match.tbd ? teamADisplay : match.teamA) ? 'checked' : ''}> ${fA} ${esc(teamADisplay)}</label>
              <label class="adm-pen-opt"><input type="radio" name="adm-pen-${match.matchId}" value="none" ${!result.penalty_winner || result.penalty_winner === 'none' ? 'checked' : ''}> None</label>
              <label class="adm-pen-opt"><input type="radio" name="adm-pen-${match.matchId}" value="${esc(match.tbd ? teamBDisplay : match.teamB)}" ${result.penalty_winner === (match.tbd ? teamBDisplay : match.teamB) ? 'checked' : ''}> ${fB} ${esc(teamBDisplay)}</label>
            </div>
            <button class="btn-calc btn-recalc" onclick="handleAdminFinalize('${match.matchId}',this)">Recalculate Points</button>
          </div>` : `<div class="adm-form">
            ...
            <div class="adm-pen-row">
              <span class="adm-pen-label">Penalty winner (if draw):</span>
              <label class="adm-pen-opt"><input type="radio" name="adm-pen-${match.matchId}" value="${esc(match.tbd ? teamADisplay : match.teamA)}"> ${fA} ${esc(teamADisplay)}</label>
              <label class="adm-pen-opt"><input type="radio" name="adm-pen-${match.matchId}" value="none" checked> None</label>
              <label class="adm-pen-opt"><input type="radio" name="adm-pen-${match.matchId}" value="${esc(match.tbd ? teamBDisplay : match.teamB)}"> ${fB} ${esc(teamBDisplay)}</label>
            </div>
...
async function handleAdminFinalize(matchId, btn) {
  ...
  try {
    const penRadio = document.querySelector(`input[name="adm-pen-${matchId}"]:checked`);
    const penWinner = penRadio ? penRadio.value : null;
    const { updated } = await finalizeMatchResult(matchId, +a.value, +b.value, penWinner);
